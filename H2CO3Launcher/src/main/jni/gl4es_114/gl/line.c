...
void gl4es_glLineStipple(GLuint factor, GLushort pattern) {
    ...
    if (pattern != glstate->linestipple.pattern || factor != glstate->linestipple.factor ||
...
GLfloat *
gen_stipple_tex_coords(GLfloat *vert, GLushort *sindices, modeinit_t *modes, int stride, int length,
                       GLfloat *noalloctex) {
    DBG(printf("Generate stripple tex (stride=%d, noalloctex=%p) length=%d:", stride, noalloctex,
               length);)
    // generate our texture coords
    GLfloat *tex = noalloctex ? noalloctex : (GLfloat *) malloc(
            modes[length - 1].ilen * 4 * sizeof(GLfloat));
    GLfloat *texPos = tex;
            ...
            for (; i < modes[k].ilen; i += 2) {
                ...
                if (sindices)
                    texPos = tex + texstride * sindices[i +
                                                        0];   // it get writen 2*, but that should be ok, it's the same value
