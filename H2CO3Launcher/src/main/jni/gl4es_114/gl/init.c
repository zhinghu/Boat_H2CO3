  ...
  SHUT_LOGD("Set gamma to %.2f\n", globals4es.gamma);
    ...
    switch (globals4es.gl) {
        ...
        default:
            ...
            globals4es.gl = (globals4es.es == 1) ? 15
                                                 : 21;  // forcing GL 1.5 for es1.1 and GL 2.1 for es2.0
            break;
    }
    ...
    switch (globals4es.texshrink) {
        case 10:
            SHUT_LOGD(
                    "Texture shink, mode 10 selected (advertise 8192 max texture size, but >2048 are quadshrinked and > 512 are shrinked), but not for empty texture\n");
            break;
        case 11:
            SHUT_LOGD(
                    "Texture shink, mode 11 selected (advertise a max texture size *2, but every texture with one dimension > max texture size will get shrinked to max texture size), but not for empty texture\n");
            break;
        case 1:
        ...
        case 5:
            SHUT_LOGD(
                    "Texture shink, mode 5 selected (every > 256 is downscaled to 256 ), but not for empty texture\n");
            break;
        case 6:
            SHUT_LOGD(
                    "Texture shink, mode 6 selected (only > 128 /2, >=512 is downscaled to 256 ), but not for empty texture\n");
            break;
        case 7:
            SHUT_LOGD(
                    "Texture shink, mode 7 selected (only > 512 /2 ), but not for empty texture\n");
            break;
        case 8:
            SHUT_LOGD(
                    "Texture shink, mode 8 selected (advertise 8192 max texture size, but >2048 are shrinked to 2048)\n");
            break;
        case 9:
            SHUT_LOGD(
                    "Texture shink, mode 9 selected (advertise 8192 max texture size, but >4096 are quadshrinked and > 512 are shrinked), but not for empty texture\n");
            break;
        default:
    ...
    env(LIBGL_BLENDHACK, globals4es.blendhack,
        "Change Blend GL_SRC_ALPHA, GL_ONE to GL_ONE, GL_ONE");
    ...
    if (env_version) {
        snprintf(globals4es.version, 49, "%s gl4es wrapper %d.%d.%d", env_version, MAJOR, MINOR,
                 REVISION);
        SHUT_LOGD("Targeting OpenGL %s\n", env_version);
    } else {
        snprintf(globals4es.version, 49, "%d.%d gl4es wrapper %d.%d.%d", globals4es.gl / 10,
                 globals4es.gl % 10, MAJOR, MINOR, REVISION);
    ...
    if (GetEnvVarBool("LIBGL_AVOID16BITS", &globals4es.avoid16bits,
                      (hardext.vendor & VEND_IMGTEC) ? 0 : 1)) {
    ...
    if (env_forcenpot == 0 &&
        (hardext.esversion == 2 && (hardext.npot == 1 || hardext.npot == 2))) {
        SHUT_LOGD("Not forcing NPOT support\n");
    } else if (env_forcenpot != 0 ||
               (hardext.esversion == 2 && (hardext.npot == 1 || hardext.npot == 2))) {
        if (hardext.npot == 3) {
            ...
            SHUT_LOGD(
                    "WARNING, No Limited or Full NPOT support in hardware, Forcing NPOT have no effect!\n");
        }
    }
    ...
    env(LIBGL_NOES2COMPAT, globals4es.noes2,
    ...
    env(LIBGL_VGPU_BACKPORT, globals4es.vgpu_backport,
        "Attempt HARD to backport shaders to #version 100")
