...
#define write_each(amod, vmod, hmod)                         \
    ...
    if ((dst_color->red == dst_color->green) && (dst_color->red == dst_color->blue)) {
        // special case
        GLfloat aa = (pixel.r + pixel.g + pixel.b) /
                     3.0f;    //*TODO* find a better formula. real luminance is not just the mean value.
...
#define write_each(amod, vmod, hmod)                         \
    ...
    pixel.r =
            (pix[0].r + pix[1].r + pix[2].r + pix[3].r + pix[4].r + pix[5].r + pix[6].r + pix[7].r +
             pix[8].r + pix[9].r + pix[10].r + pix[11].r + pix[12].r + pix[13].r + pix[14].r +
             pix[15].r) * 0.0625f;
    pixel.g =
            (pix[0].g + pix[1].g + pix[2].g + pix[3].g + pix[4].g + pix[5].g + pix[6].g + pix[7].g +
             pix[8].g + pix[9].g + pix[10].g + pix[11].g + pix[12].g + pix[13].g + pix[14].g +
             pix[15].g) * 0.0625f;
    pixel.b =
            (pix[0].b + pix[1].b + pix[2].b + pix[3].b + pix[4].b + pix[5].b + pix[6].b + pix[7].b +
             pix[8].b + pix[9].b + pix[10].b + pix[11].b + pix[12].b + pix[13].b + pix[14].b +
             pix[15].b) * 0.0625f;
    pixel.a =
            (pix[0].a + pix[1].a + pix[2].a + pix[3].a + pix[4].a + pix[5].a + pix[6].a + pix[7].a +
             pix[8].a + pix[9].a + pix[10].a + pix[11].a + pix[12].a + pix[13].a + pix[14].a +
             pix[15].a) * 0.0625f;
...
bool pixel_convert(const GLvoid *src, GLvoid **dst,
    ...
    GLuint dst_width2 = widthalign((stride ? stride : width) * pixel_sizeof(dst_format, dst_type),
                                   align);
    ...
    // BGRA1555 -> RGBA5551
    if ((src_format == GL_BGRA) && (dst_format == GL_RGBA) &&
        (dst_type == GL_UNSIGNED_SHORT_5_5_5_1) && (src_type == GL_UNSIGNED_SHORT_1_5_5_5_REV)) {
        GLushort tmp;
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
...
#else
                *(GLushort *) dst_pos = ((((int) byte_src[0]) * 77 + ((int) byte_src[1]) * 151 +
                                          ((int) byte_src[2]) * 28) & 0xff00) >> 8 |
                                        (byte_src[3] << 8);
#endif
                src_pos += src_stride;
                dst_pos += dst_stride;
...
#else
                *(GLushort *) dst_pos = ((((int) byte_src[2]) * 77 + ((int) byte_src[1]) * 151 +
                                          ((int) byte_src[0]) * 28) & 0xff00) >> 8 |
                                        (byte_src[3] << 8);
#endif
                src_pos += src_stride;
                dst_pos += dst_stride;
...
#else
                *(unsigned char *) dst_pos = (((int) byte_src[0]) * 77 + ((int) byte_src[1]) * 151 +
                                              ((int) byte_src[2]) * 28) >> 8;
#endif
                src_pos += src_stride;
                dst_pos += dst_stride;
...
#else
                *(unsigned char *) dst_pos = (((int) byte_src[2]) * 77 + ((int) byte_src[1]) * 151 +
                                              ((int) byte_src[0]) * 28) >> 8;
#endif
                src_pos += src_stride;
                dst_pos += dst_stride;
    ...
    // RGBA -> RGBA4444
    if ((src_format == GL_RGBA) && (dst_format == GL_RGBA) &&
        (dst_type == GL_UNSIGNED_SHORT_4_4_4_4) && ((src_type == GL_UNSIGNED_BYTE))) {
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                *(GLushort *) dst_pos = ((GLushort) (((char *) src_pos)[3] & 0xf0)) >> (4) |
    ...
    if (src_format == GL_COLOR_INDEX) {
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                idx = (((*((GLubyte *) src_pos)) << glstate->raster.index_shift) +
                       glstate->raster.index_offset);
...
/*    if (new_width*2!=width || new_height*2!=height) {
    ...
    for (int y = 0; y < new_height; y++) {
        for (int x = 0; x < new_width; x++) {
            ...
            pix3 = src + ((x * mx + dx) +
                          (y * my + dy) * width) * pixel_size;
            half_pixel((GLvoid *) pix0, (GLvoid *) pix1, (GLvoid *) pix2, (GLvoid *) pix3,
                       (GLvoid *) pos, src_color, type);
            pos += pixel_size;
        }
    }
    *new = dst;
...
bool pixel_thirdscale(const GLvoid *old, GLvoid **new,
    ...
    new_height = height / 2;
    if (!new_height) ++new_height;
    if (new_width * 2 != width || new_height * 2 != height || format != GL_RGBA ||
    ...
    for (int y = 0; y < new_height; y++) {
        for (int x = 0; x < new_width; x++) {
            ...
            pix3 = src + ((x * mx + dx) +
                          (y * my + dy) * width) * pixel_size;
            half_pixel((GLvoid *) pix0, (GLvoid *) pix1, (GLvoid *) pix2, (GLvoid *) pix3,
                       (GLvoid *) tmp, src_color, type);
            *((GLushort *) pos) =
                    (((GLushort) tmp[0]) & 0xf0) << 8 | (((GLushort) tmp[1]) & 0xf0) << 4 |
                    (((GLushort) tmp[2]) & 0xf0) | (((GLushort) tmp[3]) >> 4);
            pos += dest_size;
        }
    }
    *new = dst;
...
//    printf("LIBGL: quarterscaling %ux%u -> %ux%u\n", width, height, new_width, new_height);
    ...
    const int dys[4] = {0, height > 1 ? 1 : 0, height > 2 ? 2 : 0,
                        height > 3 ? 3 : height > 1 ? 1 : 0};
    for (int y = 0; y < new_height; y++) {
        for (int x = 0; x < new_width; x++) {
...
bool pixel_to_ppm(const GLvoid *pixels, GLuint width, GLuint height,
    ...
    if (format == GL_RGB && type == GL_UNSIGNED_BYTE) {
        ...
        if (!pixel_convert(pixels, (GLvoid **) &src, width, height, format, type, GL_RGB,
                           GL_UNSIGNED_BYTE, 0, align)) {
            return false;
        }
    }
