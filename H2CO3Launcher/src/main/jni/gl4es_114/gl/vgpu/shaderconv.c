...
char *ConvertShaderConditionally(struct shader_s *shader_source) {
    ...
    shader_source->converted = ConvertShader(shader_source->source,
                                             shader_source->type == GL_VERTEX_SHADER ? 1 : 0,
...
char *ConvertShaderVgpu(struct shader_s *shader_source) {
    ...
    source = ReplacePrecisionQualifiers(source, &sourceLength,
                                        shader_source->type == GL_VERTEX_SHADER);
...
char *FindAndCorrect(char *source, int *length, int mode) {
    const char *template =
            mode == MODE_SWITCH ? switch_template : mode == MODE_CASE ? case_template : NULL;
    ...
    while (1) {
        ...
        offset = string_offset +
                 (strstr(scan_source, mode == MODE_SWITCH ? "{" : mode == MODE_CASE ? ":" : 0) -
                  scan_source); // find it by hand cause sscanf has trouble with two %n operators
        string_offset += (scan_source -
                          source); // convert it from relative to scan to relative to base
        ...
        if (mode == MODE_CASE) {
            if (!isdigit(
                    template_string[0])) { // cant have a number without the first digit, and the standard doesnt permit variable names starting with numbers
                char decltemplate_formatted[VARIABLE_SIZE];
                float declared_value = 99;
                snprintf(decltemplate_formatted, VARIABLE_SIZE, declaration_template,
                         template_string, "%f");
                ...
                source = InplaceReplaceByIndex(source, length, string_offset, replace_end_offset,
                                               integer);
                rewind = 1;
            }
        }
...
char *BackportConstArrays(char *source, int *sourceLength) {
    ...
    if (tokenStart != NULL && (tokenStart - source) < variableNameStart) {
        ...
        for (int j = 0; j < arrayEntryCount; ++j) {
            ...
            source = InplaceReplaceByIndex(source, sourceLength, entryEnd, entryEnd + 1,
                                           ";}"); // +2 - 1
        ...
        source = InplaceReplaceByIndex(source, sourceLength, variableNameStop, startArray,
                                       "(int index){");
        // Remove the []
...
char *InsertExtensions(char *source, int *sourceLength) {
    ...
    source = InsertExtension(source, sourceLength, insertPoint + 1,
...
char *
InsertExtension(char *source, int *sourceLength, const int insertPoint, const char *extension) {
...
char *ReplaceModOperator(char *source, int *sourceLength) {
    ...
    for (int i = 0; i < *sourceLength; ++i) {
        ...
        source = InplaceReplaceByIndex(source, sourceLength, startIndex, endIndex,
                                       replacementString);
    ...
    for (int i = 0; i < *sourceLength; ++i) {
        ...
        if (isDigit(source[i - 1])) {
            ...
            for (int j = 1; 1; ++j) {
                ...
                if (isValidFunctionName(source[i - j]))
                    break; // Function or variable name, don't coerce
    ...
    while (parenthesesLeft > 0 || parserState == 1) {
        ...
        if (!rightOperand && operatorValue < 3 && currentValue == 3) {
            ...
            for (int j = 1; 1; ++j) {
                ...
                if (lastOperator <=
                    operatorValue) { // If so, we allow continuing and going out of the loop
                    stringIndex -= j;
                    parserState = 1;
                    break;
                }
...
char *RemoveUnsupportedExtensions(char *source) {
    ...
    for (char *extensionPtr = strstr(source, "#extension "); extensionPtr; extensionPtr = strstr(
            source, "#extension ")) {
        int i = 0;
...
char *ReplaceFunctionName(char *source, int *sourceLength, char *initialName, char *finalName) {
    for (unsigned long currentPosition = 0; 1; currentPosition += strlen(initialName)) {
        ...
        source = InplaceReplaceByIndex(source, sourceLength, currentPosition + newPosition,
                                       currentPosition + newPosition + strlen(initialName) - 1,
                                       finalName);
        currentPosition += newPosition;
    }
    return source;
...
char *ReplacePrecisionQualifiers(char *source, int *sourceLength, int isVertex) {
    ...
    for (unsigned long currentPosition = strstrPos(source, "precision ");
         currentPosition > 0; currentPosition = strstrPos(source, "precision ")) {
...
int GetClosingTokenPosition(const char *source, int initialTokenPosition) {
    return GetClosingTokenPositionTokenOverride(source, initialTokenPosition,
                                                source[initialTokenPosition]);
}
 ...
 */
int GetClosingTokenPositionTokenOverride(const char *source, int initialTokenPosition,
                                         char initialToken) {
 ...
 * @return
 */
int GetNextTokenPosition(const char *source, int initialPosition, const char token,
 ...
 * @return The shader as a string, maybe in a different memory location
 */
char *insertIntAtFunctionCall(char *source, int *sourceSize, const char *functionName,
    ...
    while (functionCallPosition != 0) {
        int openingTokenPosition = GetNextTokenPosition(source,
                                                        functionCallPosition + strlen(functionName),
                                                        '(', " \n\r\t");
        if (source[openingTokenPosition] == '(') {
            ...
            for (int argCount = 0;
                 argCount <= argumentPosition && source[startArgPos] != ')'; ++argCount) {
        ...
        unsigned long offset = strstrPos(source + functionCallPosition + strlen(functionName),
                                         functionName);
