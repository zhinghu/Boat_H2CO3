...
void gl4es_glColorPointer(GLint size, GLenum type,
    ...
    DBG((size > 4) ? printf("glColorPointer(%d, %s, %d, %p)\n", size, PrintEnum(type), stride,
                            pointer) : printf("glColorPointer(%s, %s, %d, %p)\n", PrintEnum(size),
    ...
    clone_gl_pointer(glstate->vao->vertexattrib[ATT_COLOR], size,
                     (type == GL_FLOAT) ? GL_FALSE : GL_TRUE);
}
...
void gl4es_glTexCoordPointer(GLint size, GLenum type,
    ...
    DBG(printf("glTexCoordPointer(%d, %s, %d, %p), texture.client=%d\n", size, PrintEnum(type),
               stride, pointer, glstate->texture.client);)
    if (size < 1 || size > 4) {
        errorShim(GL_INVALID_VALUE);
        return;
    }
...
void gl4es_glSecondaryColorPointer(GLint size, GLenum type,
    ...
    DBG((size > 4) ? printf("glSecondaryColorPointer(%d, %s, %d, %p)\n", size, PrintEnum(type),
                            stride, pointer) : printf("glSecondaryColorPointer(%s, %s, %d, %p)\n",
                                                      PrintEnum(size), PrintEnum(type), stride,
                                                      pointer);)
...
void glVertexPointer(GLint size, GLenum type, GLsizei stride, const GLvoid *pointer) AliasExport(
        "gl4es_glVertexPointer");

void glColorPointer(GLint size, GLenum type, GLsizei stride, const GLvoid *pointer) AliasExport(
        "gl4es_glColorPointer");

void glNormalPointer(GLenum type, GLsizei stride, const GLvoid *pointer) AliasExport(
        "gl4es_glNormalPointer");

void glTexCoordPointer(GLint size, GLenum type, GLsizei stride, const GLvoid *pointer) AliasExport(
        "gl4es_glTexCoordPointer");

void
glSecondaryColorPointer(GLint size, GLenum type, GLsizei stride, const GLvoid *pointer) AliasExport(
        "gl4es_glSecondaryColorPointer");

void glFogCoordPointer(GLenum type, GLsizei stride, const GLvoid *pointer) AliasExport(
        "gl4es_glFogCoordPointer");

void glSecondaryColorPointerEXT(GLint size, GLenum type, GLsizei stride,
                                const GLvoid *pointer) AliasExport("gl4es_glSecondaryColorPointer");

void glFogCoordPointerEXT(GLenum type, GLsizei stride, const GLvoid *pointer) AliasExport(
        "gl4es_glFogCoordPointer");
...
void glInterleavedArrays(GLenum format, GLsizei stride, const GLvoid *pointer) AliasExport(
...
void gl4es_glEnd() {
    ...
    for (int a = 0; a < hardext.maxtex; a++)
        if ((hardext.esversion == 1) && glstate->enable.texture[a] &&
            ((glstate->list.active->tex[a] == 0) &&
             !(glstate->enable.texgen_s[a] || glstate->texture.pscoordreplace[a])))
            rlMultiTexCoord4f(glstate->list.active, GL_TEXTURE0 + a, glstate->texcoord[a][0],
                              glstate->texcoord[a][1], glstate->texcoord[a][2],
                              glstate->texcoord[a][3]);
    ...
    if (glstate->list.active) {
        if (glstate->list.active->stage != STAGE_DRAW) {
            if (glstate->list.compiling && glstate->list.active) {
                glstate->list.active->lastNormal[0] = nx;
                glstate->list.active->lastNormal[1] = ny;
                glstate->list.active->lastNormal[2] = nz;
            } else if (glstate->list.pending && glstate->list.active->stage == STAGE_POSTDRAW) {
                glstate->list.active->post_normals[0] = nx;
                glstate->list.active->post_normals[1] = ny;
    ...
    if (glstate->list.active) {
        if (glstate->list.active->stage != STAGE_DRAW) {
            ...
            if (!glstate->list.pending)
                return gl4es_glNormal3f(v[0], v[1],
                                        v[2]);  // this will put the call on the stack in the current list
        } else {
...
void gl4es_glColor4f(GLfloat red, GLfloat green, GLfloat blue, GLfloat alpha) {
    if (glstate->list.active) {
        if (glstate->list.active->stage != STAGE_DRAW) {
            if (glstate->list.compiling || glstate->list.active->stage < STAGE_DRAW) {
                glstate->list.active->lastColors[0] = red;
                glstate->list.active->lastColors[1] = green;
                glstate->list.active->lastColors[2] = blue;
                glstate->list.active->lastColors[3] = alpha;
                glstate->list.active->lastColorsSet = 1;
            } else if (glstate->list.pending && glstate->list.active->stage == STAGE_POSTDRAW) {
                glstate->list.active->post_colors[0] = red;
                glstate->list.active->post_colors[1] = green;
                glstate->list.active->post_colors[2] = blue;
                glstate->list.active->post_colors[3] = alpha;
    ...
    glstate->color[0] = red;
    glstate->color[1] = green;
    glstate->color[2] = blue;
    glstate->color[3] = alpha;
}

void
glColor4f(GLfloat red, GLfloat green, GLfloat blue, GLfloat alpha) AliasExport("gl4es_glColor4f");
...
void gl4es_glSecondaryColor3f(GLfloat r, GLfloat g, GLfloat b) {
    if (glstate->list.active) {
        if (glstate->list.pending)
            ...
            glstate->list.active->lastSecondaryColors[0] = r;
            glstate->list.active->lastSecondaryColors[1] = g;
    ...
    // change the state last thing
    glstate->secondary[0] = r;
    glstate->secondary[1] = g;
...
void gl4es_glTexCoord4f(GLfloat s, GLfloat t, GLfloat r, GLfloat q) {
    if (glstate->list.active) {
        if (glstate->list.pending)
            ...
            if (hardext.esversion == 1 || glstate->glsl->program ||
                (glstate->list.begin && (glstate->list.compiling || glstate->enable.texture[0])))
                rlMultiTexCoord4f(glstate->list.active, GL_TEXTURE0, s, t, r, q);
        }
    }
    noerrorShimNoPurge();
    glstate->texcoord[0][0] = s;
    glstate->texcoord[0][1] = t;
    glstate->texcoord[0][2] = r;
    glstate->texcoord[0][3] = q;
}
...
void gl4es_glMultiTexCoord4f(GLenum target, GLfloat s, GLfloat t, GLfloat r, GLfloat q) {
    ...
    if (glstate->list.active) {
        if (glstate->list.pending)
            ...
            if (hardext.esversion == 1 || (glstate->list.begin && (glstate->list.compiling ||
                                                                   glstate->enable.texture[target -
                                                                                           GL_TEXTURE0])))
                rlMultiTexCoord4f(glstate->list.active, target, s, t, r, q);
        }
    }
    noerrorShimNoPurge();
    glstate->texcoord[target - GL_TEXTURE0][0] = s;
    glstate->texcoord[target - GL_TEXTURE0][1] = t;
    glstate->texcoord[target - GL_TEXTURE0][2] = r;
    glstate->texcoord[target - GL_TEXTURE0][3] = q;
}

void glMultiTexCoord4f(GLenum target, GLfloat s, GLfloat t, GLfloat r, GLfloat q) AliasExport(
        "gl4es_glMultiTexCoord4f");

void glMultiTexCoord4fARB(GLenum target, GLfloat s, GLfloat t, GLfloat r, GLfloat q) AliasExport(
        "gl4es_glMultiTexCoord4f");
...
void gl4es_glMultiTexCoord2fv(GLenum target, GLfloat *v) {
    ...
    glstate->texcoord[target - GL_TEXTURE0][2] = 0.f;
    glstate->texcoord[target - GL_TEXTURE0][3] = 1.f;
}
...
void ToBuffer(int first, int count) {
    ...
    for (int i = 0; i < NB_VA; i++)
        if (glstate->vao->vertexattrib[i].enabled &&
            (!valid_vertex_type(glstate->vao->vertexattrib[i].type) ||
             glstate->vao->vertexattrib[i].real_buffer)) {
            return;
        }
    ...
    if (!stride) stride = gl_sizeof(glstate->vao->vertexattrib[ATT_VERTEX].type) *
                          glstate->vao->vertexattrib[ATT_VERTEX].size;
...
void glIndexPointer(GLenum type, GLsizei stride, const GLvoid *pointer) AliasExport(
...
void
glEdgeFlagPointer(GLsizei stride, const GLvoid *pointer) AliasExport("gl4es_glEdgeFlagPointer");
...
void gl4es_glColorMask(GLboolean red, GLboolean green, GLboolean blue, GLboolean alpha) {
    ...
    if (glstate->colormask[0] == red && glstate->colormask[1] == green &&
...
void glColorMask(GLboolean red, GLboolean green, GLboolean blue, GLboolean alpha) AliasExport(
        "gl4es_glColorMask");
