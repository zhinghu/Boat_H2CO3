...
bool ispurerender_renderlist(renderlist_t *list) {
    ...
    if (list->popattribute)
        return false;
    if (list->material || list->colormat_face || list->light || list->lightmodel || list->texgen ||
        list->texenv)
        return false;
    if (list->fog_op)
        return false;
...
/*    if ((a->indices==NULL) != (b->indices==NULL))
    ...
    if ((a->set_texture == b->set_texture) &&
        ((a->texture != b->texture) || (a->target_texture != b->target_texture)))
        return false;
    if (!a->set_texture && b->set_texture)
        return false;
...
void list_add_modeinit(renderlist_t *list, GLenum mode) {
    if (list->mode_init_len + 1 >= list->mode_init_cap) {
        ...
        list->mode_inits = (modeinit_t *) realloc(list->mode_inits,
                                                  list->mode_init_cap * sizeof(modeinit_t));
    }
    list->mode_inits[list->mode_init_len].mode_init = mode;
    list->mode_inits[list->mode_init_len++].ilen = list->indices ? list->ilen : (list->cur_istart
                                                                                 ? list->cur_istart
                                                                                 : list->len);
}
...
#define copy_a_indices                                       \
    ...
    switch (a->mode) {
        ...
        case GL_TRIANGLE_STRIP:
            alloc_a_indices;
            renderlist_trianglestrip_triangles(a->indices, a->indices ? a->ilen : a->len, newind,
                                               0);
            a->mode = GL_TRIANGLES;
            copy_a_indices;
            break;
...
renderlist_t *extend_renderlist(renderlist_t *list) {
    if ((list->prev != NULL) && ispurerender_renderlist(list) &&
        islistscompatible_renderlist(list->prev, list)) {
        // append list!
        ...
        memcpy(new->lastNormal, list->lastNormal, 3 * sizeof(GLfloat));
        memcpy(new->lastSecondaryColors, list->lastSecondaryColors, 3 * sizeof(GLfloat));
        memcpy(new->lastColors, (list->post_color) ? list->post_colors : list->lastColors,
               4 * sizeof(GLfloat));
...
void resize_merger_indices(int cap) {
    ...
    glstate->merger_indices = (GLushort *) realloc(glstate->merger_indices,
                                                   glstate->merger_indice_cap * sizeof(GLushort));
}
    ...
    if (hardext.esversion == 1)    // no more adjustment needed for ES2
        for (int a = 0; a < list->maxtex; a++) {
            ...
            if ((list->tex[a]) && (itarget == ENABLED_TEXTURE_RECTANGLE) && (bound)) {
                tex_coord_rect_arb(list->tex[a], list->tex_stride[a] >> 2, list->len, bound->width,
                                   bound->height);
            }
        }
}
...
renderlist_t *end_renderlist(renderlist_t *list) {
    ...
    switch (list->mode) {
        case GL_QUADS:
            if (((list->indices) && (list->ilen == 4)) ||
                ((list->indices == NULL) && (list->len == 4))) {
...
renderlist_t *recycle_renderlist(renderlist_t *list, GLenum mode) {
    if (isempty_renderlist(list) || (ispurerender_renderlist(list) && list->len == 0)) {
        ...
        if (list->post_color) {
            list->post_color = 0;
            rlColor4f(list, list->post_colors[0], list->post_colors[1], list->post_colors[2],
                      list->post_colors[3]);
        }
        if (list->post_normal) {
            list->post_normal = 0;
...
#define post_expand  \
    ...
    switch (list->mode) {
        ...
        case GL_LINE_LOOP:
            pre_expand;
            renderlist_lineloop_lines(list->indices, list->indices ? list->ilen : list->len,
                                      indices, 0);
            post_expand;
            list->mode = GL_LINES;
            break;
        case GL_LINE_STRIP:
            pre_expand;
            renderlist_linestrip_lines(list->indices, list->indices ? list->ilen : list->len,
                                       indices, 0);
            post_expand;
            list->mode = GL_LINES;
            break;
        ...
        case GL_TRIANGLE_FAN:
            pre_expand;
            renderlist_trianglefan_triangles(list->indices, list->indices ? list->ilen : list->len,
                                             indices, 0);
            post_expand;
        ...
        case GL_TRIANGLE_STRIP:
            pre_expand;
            renderlist_trianglestrip_triangles(list->indices,
                                               list->indices ? list->ilen : list->len, indices, 0);
            post_expand;
            list->mode = GL_TRIANGLES;
            break;
        case GL_QUADS:
            pre_expand;
            renderlist_quads_triangles(list->indices, list->indices ? list->ilen : list->len,
                                       indices, 0);
            post_expand;
            list->mode = GL_TRIANGLES;
            break;
