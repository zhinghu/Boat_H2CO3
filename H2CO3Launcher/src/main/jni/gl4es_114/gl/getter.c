...
void gl4es_glGetPointerv(GLenum pname, GLvoid **params) {
    ...
    switch (pname) {
        ...
        case GL_TEXTURE_COORD_ARRAY_POINTER:
            *params = (void *) glstate->vao->vertexattrib[ATT_MULTITEXCOORD0 +
                                                          glstate->texture.client].pointer;
            break;
...
const GLubyte *gl4es_glGetString(GLenum name) {
    ...
    switch (name) {
        ...
        case GL_VENDOR:
            return (GLubyte *) "ptitSeb & https://github.com/Boat-H2CO3/Boat_H2CO3";
        case GL_RENDERER:
...
int gl4es_commonGet(GLenum pname, GLfloat *params) {
    switch (pname) {
        ...
        case GL_TEXTURE_STACK_DEPTH:
            *params = (glstate->texture_matrix) ? (
                    glstate->texture_matrix[glstate->texture.active]->top + 1) : 1;
            break;
        case GL_MAX_LIST_NESTING:
            *params = 64;    // fake, no limit in fact
            break;
        case GL_TEXTURE_BINDING_1D:
            *params = glstate->texture.bound[glstate->texture.active][ENABLED_TEX1D]
                      ? glstate->texture.bound[glstate->texture.active][ENABLED_TEX1D]->glname : 0;
            break;
        case GL_TEXTURE_BINDING_2D:
            *params = glstate->texture.bound[glstate->texture.active][ENABLED_TEX2D]
                      ? glstate->texture.bound[glstate->texture.active][ENABLED_TEX2D]->glname : 0;
            break;
        case GL_TEXTURE_BINDING_3D:
            *params = glstate->texture.bound[glstate->texture.active][ENABLED_TEX3D]
                      ? glstate->texture.bound[glstate->texture.active][ENABLED_TEX3D]->glname : 0;
            break;
        case GL_TEXTURE_BINDING_RECTANGLE_ARB:
            *params = glstate->texture.bound[glstate->texture.active][ENABLED_TEXTURE_RECTANGLE]
                      ? glstate->texture.bound[glstate->texture.active][ENABLED_TEXTURE_RECTANGLE]->glname
                      : 0;
            break;
        case GL_TEXTURE_BINDING_CUBE_MAP:
            *params = glstate->texture.bound[glstate->texture.active][ENABLED_CUBE_MAP]
                      ? glstate->texture.bound[glstate->texture.active][ENABLED_CUBE_MAP]->glname
                      : 0;
            break;
...
void
glGetMaterialfv(GLenum face, GLenum pname, GLfloat *params) AliasExport("gl4es_glGetMaterialfv");
...
void gl4es_glGetClipPlanef(GLenum plane, GLfloat *equation) {
    ...
    if (gles_glGetClipPlanef) {
        ...
        noerrorShim();
        memcpy(equation, glstate->planes[p],
               4 * sizeof(GLfloat)); // should return transformed coordinates
    }
}
