...
//printf("%sglLightModelfv(%04X, [%.2f, %.2f, %.2f, %.2f])\n", (state.list.compiling)?"list":"", pname, params[0], params[1], params[2], params[3]);
    ...
    if (glstate->list.active)
        if ((glstate->list.compiling)) {
            ...
            int sz = 4;
            if (pname == GL_LIGHT_MODEL_TWO_SIDE || pname == GL_LIGHT_MODEL_COLOR_CONTROL ||
...
void gl4es_glColorMaterial(GLenum face, GLenum mode) {
    ...
    if (mode != GL_EMISSION && mode != GL_AMBIENT && mode != GL_DIFFUSE && mode != GL_SPECULAR &&
    ...
    if (glstate->fpe_state) {
        ...
        switch (mode) {
            case GL_EMISSION:
                value = FPE_CM_EMISSION;
                break;
            case GL_AMBIENT:
                value = FPE_CM_AMBIENT;
                break;
            case GL_DIFFUSE:
                value = FPE_CM_DIFFUSE;
                break;
            case GL_SPECULAR:
                value = FPE_CM_SPECULAR;
                break;
        }
        if (face == GL_FRONT_AND_BACK || face == GL_FRONT) {
...
void
glMaterialfv(GLenum face, GLenum pname, const GLfloat *params) AliasExport("gl4es_glMaterialfv");
