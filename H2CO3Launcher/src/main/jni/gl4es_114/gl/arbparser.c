...
char **resolveParam(sCurStatus_NewVar *newVar, int vertex, int type) {
    ...
    if (!tok) {
        ...
        if (!tok) {
            ...
            if (newVar->strParts[0][0] == '[') {
                ...
                if ((sln[0] >= '0') && (sln[0] <= '9')) {
                    ...
                    matrixNameMallocd = (char *) malloc(
                            (mtxNameLen + strlen(sln) + 8) * sizeof(char));
            ...
            if (newVar->strParts[0][0] == '[') {
                ...
                if ((sln[0] >= '0') && (sln[0] <= '9')) {
                    ...
                    matrixNameMallocd = (char *) malloc(
                            (mtxNameLen + slnLen + propLen + 4) * sizeof(char));
            ...
            if (!tok) {
                ...
                if (newVar->strLen && !IS_NEW_STR_OR_SWIZZLE(newVar->strParts[0], type)) {
                    ...
                    if (newVar->strLen && !IS_NEW_STR_OR_SWIZZLE(newVar->strParts[0], type)) {
                        tok = popFIFO((sArray *) newVar);
                        if (!strcmp(tok, "row")
                            && newVar->strLen && (newVar->strParts[0][0] >= '0') &&
                            (newVar->strParts[0][0] <= '9')) {
                            free(tok);
                            tok = popFIFO((sArray *) newVar);
                            for (char *numPtr = tok; *numPtr; ++numPtr) {
                            ...
                            if ((newVar->strLen >= 3) && (newVar->strParts[0][0] == '.')
                                && (newVar->strParts[0][1] == '.')
                                && (newVar->strParts[1][0] >= '0') &&
                                (newVar->strParts[1][0] <= '9')) {
                                end = 0;
                                free(popFIFO((sArray *) newVar));
                                tok = popFIFO((sArray *) newVar);
                                for (char *numPtr = tok; *numPtr; ++numPtr) {
        ...
        do {
            ...
            switch (pseudoSt.curToken) {
                case TOK_SIGN:
                    if ((pseudoSt.curValue.newVar.state % 3) ||
                        appendString(&pseudoSt, pseudoSt.tokInt ? "+" : "-", 1)) {
                ...
                case TOK_COMMA:
                    if ((pseudoSt.curValue.newVar.state % 3 != 2) ||
                        (pseudoSt.curValue.newVar.state > 8)) {
                        pseudoSt.status = ST_ERROR;
                        continue;
                    }
        ...
        if (pseudoSt.status == ST_ERROR) {
            ARBCONV_DBG_RE("Failed to get param: %s tok (%s))\n", TOKEN2STR(pseudoSt.curToken),
                           pseudoSt.outputString)
    ...
    if (isMatrix) {
        ...
        for (int i = start; i <= end; ++i) {
            char *buf = (char *) calloc(mtxNameLen + 13,
                                        sizeof(char)); // Assume 32-bit array = 11 digits max
        ...
        case ST_LINE_START:
            switch (curStatusPtr->curToken) {
                case TOK_IDENTIFIER: {
                    ...
                    eInstruction inst;
                    eVariableType vtype;
                    if ((inst = STR2INST(tok, &curStatusPtr->curValue.newInst.inst.saturated)) !=
                        INST_UNK) {
                ...
                case VARTYPE_TEMP:
                    switch (curStatusPtr->curToken) {
                        ...
                        case TOK_COMMA: {
                            ...
                            kh_val(curStatusPtr->varsMap,
                                   varIdx) = curStatusPtr->curValue.newVar.var;
                            pushArray((sArray *) &curStatusPtr->variables,
                                      curStatusPtr->curValue.newVar.var);
                            curStatusPtr->curValue.newVar.var = createVariable(
                                    curStatusPtr->curValue.newVar.var->type);
                            curStatusPtr->curValue.newVar.state = 0;
                            break;
                        }
            ...
            switch (curStatusPtr->curValue.newVar.var->type) {
                case VARTYPE_ATTRIB:
                    switch (curStatusPtr->curToken) {
                        case TOK_IDENTIFIER: {
                            ...
                            pushArray((sArray *) &curStatusPtr->curValue.newVar,
                                      getToken(curStatusPtr));
				...
				if (curStatusPtr->curValue.newVar.state != 2) {
					FAIL("Invalid state");
                            ...
                            if ((curStatusPtr->curValue.newVar.state < 2) ||
                                (curStatusPtr->curValue.newVar.state > 10)
                                || (curStatusPtr->curValue.newVar.state % 2)) {
                                FAIL("Invalid state");
                            }
                        ...
                        case TOK_COMMA:
                            if ((curStatusPtr->curValue.newVar.state < 5) ||
                                (curStatusPtr->curValue.newVar.state > 9)
                                || !(curStatusPtr->curValue.newVar.state % 2)) {
                                FAIL("Invalid state");
                            }
                        ...
                        case TOK_RBRACE:
                            if ((curStatusPtr->curValue.newVar.state != 5) &&
                                (curStatusPtr->curValue.newVar.state != 11)) {
                                FAIL("Invalid state");
                            }
                        ...
                        case TOK_END_OF_INST:
                            ...
                            for (char **parm = param; *parm; ++parm) {
                                pushArray((sArray *) &curStatusPtr->curValue.newVar.var->init,
                                          *parm);
                            }
                            free(param);
                ...
                case VARTYPE_PARAM_MULT:
                    switch (curStatusPtr->curToken) {
                        ...
                        case TOK_COMMA:
                            if (curStatusPtr->curValue.newVar.state == 2) {
                                pushArray((sArray *) &curStatusPtr->curValue.newVar,
                                          getToken(curStatusPtr));
                                curStatusPtr->curValue.newVar.state = 1;
                            } else if ((curStatusPtr->curValue.newVar.state > 7) &&
                                       !(curStatusPtr->curValue.newVar.state % 2)) {
                                pushArray((sArray *) &curStatusPtr->curValue.newVar,
                                          getToken(curStatusPtr));
                                ++curStatusPtr->curValue.newVar.state;
                        ...
                        case TOK_RSQBRACKET:
                            if ((curStatusPtr->curValue.newVar.state != 4) &&
                                (curStatusPtr->curValue.newVar.state != 6)) {
                                FAIL("Invalid state");
                            }
                        ...
                        case TOK_END_OF_INST:
                            ...
                            do {
                                ...
                                char **param = resolveParam(&curStatusPtr->curValue.newVar, vertex,
                                                            2);
                                ...
                                for (char **parm = param; *parm; ++parm) {
                                    ARBCONV_DBG_HEAVY(printf("Resolved param %p: ", *parm);fflush(
                                            stdout);printf("%s\n", *parm);)
                                    pushArray((sArray *) &curStatusPtr->curValue.newVar.var->init,
                                              *parm);
                                }
                            ...
                            if ((curStatusPtr->status == ST_ERROR) ||
                                curStatusPtr->curValue.newVar.strLen) {
                    ...
                    switch (curStatusPtr->curValue.newInst.state) {
                        ...
                        case STATE_LSQBR_SIGN: {
                            char *faa = (char *) realloc(
                                    ...
                                    (strlen(curVarPtr->floatArrAddr) +
                                     getTokenLength(curStatusPtr) + 1) * sizeof(char)
                            );
                            if (!faa) {
                ...
                case TOK_INTEGER:
                    switch (curStatusPtr->curValue.newInst.state) {
                        ...
                        case STATE_LSQBR_START:
                            pushArray((sArray *) &curStatusPtr->_fixedNewVar,
                                      getToken(curStatusPtr));
                ...
                case TOK_IDENTIFIER: {
                    ...
                    switch (curStatusPtr->curValue.newInst.state) {
                        ...
                        case STATE_AFTER_TEXSPLINT:
                            if (texSampler) {
                                if ((getTokenLength(curStatusPtr) != 1) ||
                                    strncmp(curStatusPtr->codePtr, "D", 1)) {
                                    FAIL("Invalid texture sampler");
                                } else {
                        ...
                        case STATE_AFTER_ELEMENT:
                            if (INSTTEX(curStatusPtr->curValue.newInst.inst.type)
                                ...
                                if (!curStatusPtr->texVars[id]->size) {
                                    pushArray((sArray *) curStatusPtr->texVars[id],
                                              strdup(curStatusPtr->_fixedNewVar.strParts[2]));
                                }
                                ...
                                if ((vertex &&
                                     !strcmp(curStatusPtr->_fixedNewVar.strParts[0], "vertex"))
                                    || (!vertex && !strcmp(curStatusPtr->_fixedNewVar.strParts[0],
                                                           "fragment"))) {
                                    failure = resolveAttrib(&curStatusPtr->_fixedNewVar, vertex);
                                } else if (!strcmp(curStatusPtr->_fixedNewVar.strParts[0],
                                                   "result")) {
                                    failure = resolveOutput(&curStatusPtr->_fixedNewVar, vertex,
                                                            hasFogFragCoord);
                                } else {
                                    char **resolved = resolveParam(&curStatusPtr->_fixedNewVar,
                                                                   vertex, 1);
                                    ...
                                    for (char **resvd = resolved; *resvd; ++resvd) {
                                        pushArray((sArray *) &curStatusPtr->_fixedNewVar.var->init,
                                                  *resvd);
                                    }
                                    free(resolved);
                                }
...
#undef STATE_AFTER_SWIZZLE
        ...
        case ST_OPTION:
            switch (curStatusPtr->curToken) {
                ...
                case TOK_END_OF_INST:
                    if (!strcmp(curStatusPtr->curValue.newOpt.optName,
                                "ARB_precision_hint_fastest")) {
                        // Nothing to do
                    } else if (!strcmp(curStatusPtr->curValue.newOpt.optName,
                                       "ARB_precision_hint_nicest")) {
                        // Nothing to do
                    } else if (!vertex &&
                               !strcmp(curStatusPtr->curValue.newOpt.optName, "ARB_fog_exp")) {
                        curStatusPtr->fogType = FOG_EXP;
                    } else if (!vertex &&
                               !strcmp(curStatusPtr->curValue.newOpt.optName, "ARB_fog_exp2")) {
                        curStatusPtr->fogType = FOG_EXP2;
                    } else if (!vertex &&
                               !strcmp(curStatusPtr->curValue.newOpt.optName, "ARB_fog_linear")) {
                        curStatusPtr->fogType = FOG_LINEAR;
                    } else if (vertex && !strcmp(curStatusPtr->curValue.newOpt.optName,
