...
#define clientGO_proxyFPE(constant, name) \
    ...
    switch (cap) {
        ...
        // texgen
        GOFPE(GL_TEXTURE_GEN_S, texgen_s[glstate->texture.active],
              fpe_changetexgen_s(glstate->texture.active, enable)); //TODO: FPE stuffs
        GOFPE(GL_TEXTURE_GEN_T, texgen_t[glstate->texture.active],
              fpe_changetexgen_t(glstate->texture.active, enable));
        GOFPE(GL_TEXTURE_GEN_R, texgen_r[glstate->texture.active],
              fpe_changetexgen_r(glstate->texture.active, enable));
        GOFPE(GL_TEXTURE_GEN_Q, texgen_q[glstate->texture.active],
              fpe_changetexgen_q(glstate->texture.active, enable));
        ...
        // point sprite
        proxy_GOFPE(GL_POINT_SPRITE, pointsprite,
                    glstate->fpe_state->pointsprite = enable); // TODO: plugin fpe stuffs
        ...
        case GL_TEXTURE_COORD_ARRAY:
            change_vao_texcoord(glstate->texture.client, enable);
            break;
        ...
        GONF(GL_MAP2_VERTEX_4, map2_vertex4);
            ...
            // ARB_vertex_program
        proxy_GOFPE(GL_VERTEX_PROGRAM_ARB, vertex_arb,
                    glstate->fpe_state->vertex_prg_enable = enable);
        GONF(GL_VERTEX_PROGRAM_TWO_SIDE_ARB, vertex_two_side_arb);
            // ARB_fragment_program
        proxy_GOFPE(GL_FRAGMENT_PROGRAM_ARB, fragment_arb,
                    glstate->fpe_state->fragment_prg_enable = enable);
        ...
        case GL_TEXTURE_2D:
            if (glstate->list.pending &&
                ((glstate->enable.texture[glstate->texture.active] >> ENABLED_TEX2D) & 1) != enable)
                gl4es_flush();
        ...
        case GL_TEXTURE_RECTANGLE_ARB:
            FLUSH_BEGINEND;
            if (enable)
                glstate->enable.texture[glstate->texture.active] |= (1
...
GLboolean gl4es_glIsEnabled(GLenum cap) {
    DBG(printf("glIsEnabed(%s), list.begin=%d, list.compiling=%d, list.pending=%d\n",
               PrintEnum(cap), glstate->list.begin, glstate->list.compiling,
    ...
    switch (cap) {
        ...
        case GL_TEXTURE_1D:
            return glstate->enable.texture[glstate->texture.active] & (1 << ENABLED_TEX1D);
        case GL_TEXTURE_2D:
            return glstate->enable.texture[glstate->texture.active] & (1 << ENABLED_TEX2D);
        case GL_TEXTURE_3D:
            return glstate->enable.texture[glstate->texture.active] & (1 << ENABLED_TEX3D);
        case GL_TEXTURE_CUBE_MAP:
            return glstate->enable.texture[glstate->texture.active] & (1 << ENABLED_CUBE_MAP);
        ...
        clientisenabled(GL_TEXTURE_COORD_ARRAY,
                        vertexattrib[ATT_MULTITEXCOORD0 + glstate->texture.client].enabled);
