...
static inline void rlVertexCommon(renderlist_t *list, int idx, int l) {
    if (list->use_glstate) {
        ...
        if (list->fogcoord)
            memcpy(list->fogcoord + (l * 1), glstate->fogcoord, sizeof(GLfloat) * 1);
    }
    // common part
...
void rlEnd(renderlist_t *list) {
    ...
    list->cur_istart = 0;
    if (list->mode_inits)
        list_add_modeinit(list, list->merger_mode ? list->merger_mode : list->mode_init);
    if (list->color)
    ...
    if (list->secondary == NULL) {
        if (list->use_glstate) {
            if (!glstate->merger_secondary)
                glstate->merger_secondary = (GLfloat *) malloc(
                        sizeof(GLfloat) * 4 * glstate->merger_cap);
        ...
        if (list->use_glstate) {
            if (tmu < 2)
                ...
                if (!glstate->merger_tex[tmu - 2])
                    glstate->merger_tex[tmu - 2] = (GLfloat *) malloc(
                            sizeof(GLfloat) * 4 * glstate->merger_cap);
...
void FASTMATH
rlMultiTexCoord4f(renderlist_t *list, GLenum target, GLfloat s, GLfloat t, GLfloat r, GLfloat q) {
...
renderlist_t *NewDrawStage(renderlist_t *l, GLenum m) {
    if (globals4es.mergelist && !glstate->polygon_mode
        && ((isempty_renderlist(l) && l->prev && l->prev->open && l->prev->mode == m &&
             l->prev->mode_init == m)
        ...
        if (!glstate->merger_used && !glstate->list.compiling
            && !(glstate->render_mode == GL_SELECT || glstate->polygon_mode == GL_LINE ||
                 glstate->polygon_mode == GL_POINT)) {
