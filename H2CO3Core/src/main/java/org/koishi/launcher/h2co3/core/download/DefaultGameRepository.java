...
import org.koishi.launcher.h2co3.core.download.game.VersionJsonSaveTask;
...
import org.koishi.launcher.h2co3.core.game.GameDirectoryType;
...
import org.koishi.launcher.h2co3.core.utils.event.EventBus;
import org.koishi.launcher.h2co3.core.utils.event.GameJsonParseFailedEvent;
import org.koishi.launcher.h2co3.core.utils.event.LoadedOneVersionEvent;
import org.koishi.launcher.h2co3.core.utils.event.RefreshedVersionsEvent;
import org.koishi.launcher.h2co3.core.utils.event.RefreshingVersionsEvent;
import org.koishi.launcher.h2co3.core.utils.event.RemoveVersionEvent;
import org.koishi.launcher.h2co3.core.utils.event.RenameVersionEvent;
...
public class DefaultGameRepository implements GameRepository {
    ...
    public Version readVersionJson(File file) throws IOException, JsonParseException {
        String jsonText = FileTools.readText(file);
        try {
    ...
    public boolean renameVersion(String from, String to) {
        ...
        try {
            ...
            FileTools.writeText(toJson.toFile(), JsonUtils.GSON.toJson(fromVersion.setId(to)));
            ...
            for (Version version : getVersions()) {
                if (from.equals(version.getInheritsFrom())) {
                    ...
                    FileTools.writeText(json, JsonUtils.GSON.toJson(version.setInheritsFrom(to)));
                }
            }
            return true;
    ...
    protected void refreshVersionsImpl() {
        ...
        if (files != null)
            Arrays.stream(files).parallel().filter(File::isDirectory).flatMap(dir -> {
                ...
                if (!json.exists()) {
                    List<File> jsons = FileTools.listFilesByExtension(dir, "json");
                    if (jsons.size() == 1) {
                        ...
                        File jar = new File(dir, FileTools.getNameWithoutExtension(jsons.get(0)) + ".jar");
    ...
    public AssetIndex getAssetIndex(String version, String assetId) throws IOException {
        try {
            return Objects.requireNonNull(JsonUtils.GSON.fromJson(FileTools.readText(getIndexFile(version, assetId)), AssetIndex.class));
    ...
    protected Path reconstructAssets(String version, String assetId) throws IOException, JsonParseException {
        ...
        String assetIndexContent = FileTools.readText(indexFile);
    ...
    public <M> ModpackConfiguration<M> readModpackConfiguration(String version) throws IOException, VersionNotFoundException {
        ...
        return JsonUtils.GSON.fromJson(FileTools.readText(file), new TypeToken<ModpackConfiguration<M>>() {
        }.getType());
    }
