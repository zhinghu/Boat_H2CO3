...
public final class CurseCompletionTask extends Task<Void> {
        ...
        if (manifest == null)
            try {
                ...
                if (manifestFile.exists())
                    this.manifest = JsonUtils.GSON.fromJson(FileTools.readText(manifestFile), CurseManifest.class);
            } catch (Exception e) {
    ...
    public void execute() throws Exception {
        ...
        FileTools.writeText(new File(root, "manifest.json"), JsonUtils.GSON.toJson(newManifest));
        ...
        dependencies = newManifest.getFiles()
                ...
                .flatMap(f -> {
                    try {
                        ...
                        return Stream.of(task.withCounter("h2co3.modpack.download"));
                    } catch (IOException e) {
